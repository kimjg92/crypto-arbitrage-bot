...
class MexcExchange(BaseExchange):
    ...
    async def init(self):
        ...
        self.exchange = ccxt.mexc(cfg)
        await self.exchange.load_markets()
        logger.info("MEXC 연결 완료")

    def _make_futures(self):
        cfg = make_exchange_config(self.api_key, self.api_secret, {"defaultType": "swap"})
        return ccxt.mexc(cfg)

    async def get_futures_funding_rate(self, symbol: str) -> dict:
        futures = self._make_futures()
        try:
            await futures.load_markets()
            # MEXC 선물 심볼: BTC/USDT → BTC/USDT:USDT
            ...
            logger.debug(f"MEXC 펀딩피 조회 실패 {symbol}: {e}")
            return {"symbol": symbol, "exchange": "mexc", "funding_rate": 0.0, "next_funding_time": "N/A"}
        finally:
            await futures.close()
    ...
    async def get_futures_price(self, symbol: str) -> float:
        ...
        try:
            ...
            logger.debug(f"MEXC 선물가 조회 실패 {symbol}: {e}")
    ...
    async def place_spot_order(self, symbol: str, side: str, amount: float) -> dict:
        ...
        logger.info(f"[MEXC 현물] {side.upper()} {symbol} {amount:.6f} - ID: {order['id']}")
        return order
    ...
    async def place_futures_order(self, symbol: str, side: str, amount: float) -> dict:
        ...
        try:
            ...
            logger.info(f"[MEXC 선물] {side.upper()} {symbol} - ID: {order['id']}")
            return order
        finally:
            await futures.close()
