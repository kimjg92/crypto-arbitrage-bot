...
from src.exchanges.base import BaseExchange, make_exchange_config, make_session
...
class BinanceExchange(BaseExchange):
    ...
    async def place_spot_order(self, symbol: str, side: str, amount: float) -> dict:
        ...
        logger.info(f"[Binance 현물] {side.upper()} {symbol} {amount:.6f} - ID: {order['id']}")
        return order
    ...
    async def place_futures_order(self, symbol: str, side: str, amount: float) -> dict:
        ...
        try:
            ...
            logger.info(f"[Binance 선물] {side.upper()} {symbol} {amount:.6f} - ID: {order['id']}")
            return order
        finally:
            await futures.close()
